...
public
	ref class CustomerForm : public Form
	{
	public:
		CustomerForm()
		{
			InitializeComponent(); // Initialize customer ID for subsequent actions
		}

		void LoadCustomerData(DataGridView ^ grid);		// Load data from encrypted file into DataGridView
		void SaveCustomerData(DataGridView ^ grid);		// Save data to encrypted file
		void ModifyCustomerData(DataGridView ^ grid); // Modify data in encrypted file
		bool IsFileOpen(String ^ filePath);						// Check if file is open by another program

	public:
		property Customer ^ SelectedCustomer; // Selected customer property
	...
	private:
		System::Windows::Forms::TextBox ^ clientEmailText;

	private:
		System::Windows::Forms::TextBox ^ clientPhoneText;

	private:
		System::Windows::Forms::TextBox ^ clientAddressText;

	private:
		System::Windows::Forms::TextBox ^ clientNameText;

	private:
		System::Windows::Forms::GroupBox ^ actionControlBox;

	private:
		System::Windows::Forms::Button ^ modifyClientButton;

	private:
		System::Windows::Forms::Button ^ deleteClientButton;

	private:
		System::Windows::Forms::Button ^ addClientButton;
	...
	private:
		System::Windows::Forms::Label ^ clientCodeLabel;

	private:
		System::Windows::Forms::TextBox ^ clientCodeText;

	private:
		System::Windows::Forms::Label ^ saveReminderLabel;

	private:
		System::Windows::Forms::Button ^ clearButton;

	private:
		System::Windows::Forms::Button ^ selectClientButton;

		// Variable de ruta del archivo CSV
	private:
		String ^ filePath = "data\\Clientes.csv";
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->clientEmailLabel->Text = L"Correo electr�nico:";
			//
			// clientPhoneLabel
			//
			...
			this->clientAddressLabel->Text = L"Direcci�n:";
			//
			// clientFullNameLabel
			//
			...
			this->saveChangesButton->Click += gcnew System::EventHandler(this, &CustomerForm::saveChangesButton_Click);
			//
			// modifyClientButton
			//
			...
			this->modifyClientButton->Click += gcnew System::EventHandler(this, &CustomerForm::modifyClientButton_Click);
			//
			// deleteClientButton
			//
			...
			this->Text = L"M�dulo de clientes";
...
#pragma endregion
	...
	private:
		Void CustomerForm_Load(System::Object ^ sender, System::EventArgs ^ e)
		{
			LoadCustomerData(clientsDataGridView);
		}

	private:
		Void clientsDataGridView_SelectionChanged(System::Object ^ sender, System::EventArgs ^ e)
		{
			if (clientsDataGridView->SelectedRows->Count > 0)
			{
				...
				if (selectedRow != nullptr && selectedRow->Cells[0]->Value != nullptr)
				{
					...
					{
						// Cargar la informaci�n del cliente en los campos de texto
				...
				else
				{
					...
					MessageBox::Show("Error: La fila seleccionada no contiene datos v�lidos.", "Error", MessageBoxButtons::OK, MessageBoxIcon::Error);
				}
			}
		}
	...
	private:
		Void addClientButton_Click(System::Object ^ sender, System::EventArgs ^ e)
		{
			...
			{
				MessageBox::Show("El formato del correo electr�nico no es v�lido. Debe ser de la forma 'usuario@dominio.com'.", "Error", MessageBoxButtons::OK, MessageBoxIcon::Error);
				return;
			}

			// Validaci�n del n�mero de tel�fono para que sean exactamente 8 d�gitos
			if (!Regex::IsMatch(phone, "^\\d{8}$"))
			{
				MessageBox::Show("El n�mero de tel�fono debe contener exactamente 8 d�gitos.", "Error", MessageBoxButtons::OK, MessageBoxIcon::Error);
				return;
			}
			...
			// Agregamos la nueva l�nea al DataGridView
	...
	private:
		Void modifyClientButton_Click(Object ^ sender, EventArgs ^ e)
		{
			...
			if (clientsDataGridView->SelectedRows->Count > 0)
			{
				...
				// Validaci�n del formato de correo electr�nico con expresi�n regular
				if (!Regex::IsMatch(email, "^\\S+@\\S+\\.\\S+$"))
				{
					MessageBox::Show("El formato del correo electr�nico no es v�lido. Debe ser de la forma 'usuario@dominio.com'.", "Error", MessageBoxButtons::OK, MessageBoxIcon::Error);
					return;
				...
				// Validaci�n del n�mero de tel�fono para que sean exactamente 8 d�gitos con expresion regular
				if (!Regex::IsMatch(phone, "^\\d{8}$"))
				{
					MessageBox::Show("El n�mero de tel�fono debe contener exactamente 8 d�gitos.", "Error", MessageBoxButtons::OK, MessageBoxIcon::Error);
					return;
	...
	private:
		...
		{
			// Guardar los cambios en el archivo CSV utilizando el m�todo existente
			SaveCustomerData(clientsDataGridView);
		}

	private:
		Void clientsDataGridView_CellClick(Object ^ sender, Windows::Forms::DataGridViewCellEventArgs ^ e)
		{
			// Obtener la fila seleccionada
			DataGridViewRow ^ selectedRow = clientsDataGridView->Rows[e->RowIndex];

			// Obtener los valores de la fila seleccionada
			String ^ codigoCliente = safe_cast<String ^>(selectedRow->Cells["C�digoCliente"]->Value);
			String ^ nombre = safe_cast<String ^>(selectedRow->Cells["Nombre"]->Value);
			String ^ direccion = safe_cast<String ^>(selectedRow->Cells["Direcci�n"]->Value);
			String ^ numeroTelefono = safe_cast<String ^>(selectedRow->Cells["N�meroTel�fono"]->Value);
			String ^ correoElectronico = safe_cast<String ^>(selectedRow->Cells["CorreoElectr�nico"]->Value);
	...
	public:
		Void selectClientButton_Click(Object ^ sender, EventArgs ^ e)
		{
			if (clientsDataGridView->SelectedRows->Count > 0)
			{
				DataGridViewRow ^ selectedRow = clientsDataGridView->SelectedRows[0];
				int codigoCliente = Convert::ToInt32(selectedRow->Cells["C�digoCliente"]->Value);
				String ^ nombre = safe_cast<String ^>(selectedRow->Cells["Nombre"]->Value);
				String ^ direccion = safe_cast<String ^>(selectedRow->Cells["Direcci�n"]->Value);
				String ^ telefono = safe_cast<String ^>(selectedRow->Cells["N�meroTel�fono"]->Value);
				String ^ correoElectronico = safe_cast<String ^>(selectedRow->Cells["CorreoElectr�nico"]->Value);
...
public
	ref class ClientSelectedEventArgs : public EventArgs
	{
	public:
